...
const RandomWalk = (grid: gridI, startNode: posI, finishNode: posI) => {
...
const getRandomNeighbour = (node: nodeI, grid: gridI) => {
  const neighbours = [];
  ...
  const neighboursFilteredStartAndWall = neighbours.filter(
    (neighbour) => !neighbour.isStart && !neighbour.isWall
  );
  const neighboursFilteredVisited = neighboursFilteredStartAndWall.filter(
...
const numNodesVisited = (grid: gridI) => {
  let count = 0;
